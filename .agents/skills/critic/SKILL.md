...
source of truth and is portable — it holds for any model or tool. This skill
is only the dispatch procedure.
...
Launch a **fresh** subagent (e.g. via `invoke_subagent`). Never continue an
existing agent's conversation — a critic that watched the work happen has
already been convinced.

The verbatim prompt is in `.agents/skills/critic/references/critic-prompt.md`.
Use it as it stands, pasting the content under review where marked.
