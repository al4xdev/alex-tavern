...
def _build_system_prompt(character_ids: list[str], narrator_directives: str = "") -> str:
    ...
    prompt = (
        ...
        '  "witness_ids": the IDs of every present character who could genuinely\n'
        "  perceive it given the zones, distance, and noise. Never include someone\n"
        "  who could not perceive it.}\n"
        ...
        "- DIALOGUE OWNERSHIP: you decide WHAT is said aloud; the character decides the\n"
        "  WORDS. An \"audible_speech\" event states, in reported form, the fact that\n"
        "  the character makes public this beat (\"anuncia que a selecao comeca ao\n"
        "  terceiro sino\"), never a quoted line and never a sentence written in their\n"
        "  voice. The engine hands that brief to the character's own agent, who says\n"
        "  it their way; what you write there is never recorded as their speech. For\n"
        "  someone already in next_speakers it rides along in the reply they are\n"
        "  making anyway; for anyone else it costs them a call of their own, so raise\n"
        "  one only when the room genuinely needs that fact spoken now.\n"
