...
def _build_system_prompt(character_ids: list[str], narrator_directives: str = "") -> str:
    ...
    prompt = (
        ...
        "  character makes public this beat (\"anuncia que a selecao comeca ao terceiro\n"
        "  sino\"), never a quoted line and never a sentence written in their voice.\n"
        "  engine hands that brief to the character's own agent, who says it their way;\n"
        "  what you write there is never recorded as their speech. For someone already\n"
        "  in next_speakers it rides along in the reply they are making anyway; for\n"
        "  anyone else it costs them a call of their own, so raise one only when the\n"
        "  room genuinely needs that fact spoken now.\n"
