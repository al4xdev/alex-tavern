...
CH = {
    "C1": Character(mind=CharacterMind(name="Rui", personality="Viajante cansado.", knowledge=[], current_mood="entediado"),
                    body=CharacterBody(name="Rui", physical_description="Botas sujas.", outfit="Capa")),
    "C2": Character(mind=CharacterMind(name="Marta", personality="Estalajadeira faladeira que puxa os outros para a conversa.", knowledge=["Ouviu lobos mais cedo"], current_mood="inquieta"),
                    body=CharacterBody(name="Marta", physical_description="Avental.", outfit="Avental")),
    "C3": Character(mind=CharacterMind(name="Bento", personality="Cacador pratico que age quando algo acontece.", knowledge=["A trilha do norte esta perigosa"], current_mood="alerta"),
...
async def main():
    ...
    burst_logs = [json.loads(l) for l in session_debug_path(sid).read_text().splitlines() if '"autonomous_burst"' in l]
