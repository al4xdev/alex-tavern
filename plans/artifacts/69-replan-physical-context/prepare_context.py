...
BASE = Path(__file__).resolve().parents[1] / "77-p3-input-dispatch"
...
for record in state["history"][:56]:
...
reader = OUT / "context.md"
with reader.open("x") as stream:
    stream.write("\n\n".join(lines) + "\n")
(OUT / "reader-source.json").write_text(
    json.dumps(
        {
...
print("Saved pre-replan visible fiction")
