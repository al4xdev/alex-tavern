...
out = root / "plans/artifacts/69-evacuation-input-replay"
source = root / "plans/artifacts/repetition-battery/base-P1-r2/sessions/bb72dc94/state.json"
...
assert len(paths) == 8
random.Random(691006).shuffle(paths)
key = {f"V{i + 1}": str(path.relative_to(out)) for i, path in enumerate(paths)}
