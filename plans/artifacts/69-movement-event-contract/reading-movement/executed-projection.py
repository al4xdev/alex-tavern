...
parser.add_argument("case", choices=("movement",))
args = parser.parse_args()
base = root / "plans/artifacts/69-movement-event-contract"
...
assert len(paths) == 8
random.Random(694106).shuffle(paths)
...
for label, path in zip(key, paths, strict=True):
    ...
    for event in parsed["perception_events"]:
        if cid not in event["witness_ids"]:
            continue
        subject = event["subject_id"]
        label_subject = names.get(subject, subject)
        lines.append(label_subject + ": " + canonical(event["content"]))
