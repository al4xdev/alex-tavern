...
FOOTER = (
    "This beat is a plan, not an event happening now. Confirmed HISTORY outcomes "
    "and CURRENT SCENE take precedence over it. Continue their consequences; "
    "an ending already reached stays reached. Do not replay completed events, "
    "repeat an order as newly issued, or reverse completed actions to satisfy "
    "this plan. A change of course needs a new cause in the scene."
)
...
def prepare() -> None:
    ...
    prior = ROOT / "plans/artifacts/69-anchor-heading-replay/manifest.json"
    manifest = json.loads(prior.read_text())
    for case in manifest["cases"]:
        original = case["arms"]["A"]
        candidate = copy.deepcopy(original)
        removed = copy.deepcopy(original)
        substitutions = 0
        for source, changed, ablated in zip(
            original["messages"], candidate["messages"], removed["messages"], strict=True
        ):
            if "ROTEIRO (private story direction" not in source["content"]:
                continue
            text = source["content"]
            footer = next(
                line
                for line in text.splitlines()
                if line.startswith("    This beat may span turns:")
            )
            assert text.count(OLD) == 1 and text.count(footer) == 1
            changed["content"] = text.replace(OLD, NEW).replace(footer, "    " + FOOTER)
            start = text.index("ROTEIRO (private story direction")
            end = text.index(footer) + len(footer)
            ablated["content"] = text[:start] + text[end:]
            substitutions += 1
        assert substitutions == 1
        case["arms"] = {"A": original, "B": candidate, "C": removed}
    manifest["seed"] = 692105
    manifest["parent_manifest_sha256"] = digest(prior.read_bytes())
    write(path, manifest)
    print("Prepared three arms on two frozen source requests")
