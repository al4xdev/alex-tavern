...
PREREG = HERE / "BLUE-GATE-ACTION-PREREGISTRATION.md"
MANIFEST = HERE / "blue-gate-action-manifest.json"
RUNS = HERE / "blue-gate-action-runs"
ID_PATTERN = re.compile(r"\bC\d+\b")
TURNS = (36, 37, 38, 39)
SCHEMA: dict[str, Any] = {
    "type": "object",
    "properties": {
        "closing_action": {"type": "boolean"},
        "evidence_quote": {"type": "string"},
    },
    "required": ["closing_action", "evidence_quote"],
...
def public_projection(response_text: str) -> dict[str, Any]:
    ...
    projection = {
        "events": [event["content"] for event in response["perception_events"]],
...
def prompt(turn: int, projection: dict[str, Any]) -> list[dict[str, str]]:
    system = (
        "You read accepted Director events for one beat. The target is the blue dungeon gate. "
        "Read the events in order, then time_skip_summary as a final confirmed "
        "event when nonempty. "
        "Set closing_action true only when the text asserts that this target gate FINISHES "
        "CLOSING during this beat. Set it false for a narrowing but still usable gap, "
        "a warning of future closure, or observation that the gate is already sealed. "
        "Judge the proposed action independently of whether the gate may already be closed. "
        "Copy one nonempty evidence_quote verbatim from one supplied event "
        "or the time_skip_summary. "
        "Do not quote scene state, later narration or unstated facts. Return only one JSON object "
        "matching this schema: " + json.dumps(SCHEMA, ensure_ascii=False, separators=(",", ":"))
    )
    user = json.dumps(
        {
            "turn": f"T{turn}",
            "target": "blue dungeon gate",
            "proposal": projection,
        },
        ensure_ascii=False,
        indent=2,
        ...
        cases.append(
            {
                "turn": turn,
                "source_line": line,
                "event_texts": projection["events"]
                + ([projection["time_skip_summary"]] if projection["time_skip_summary"] else []),
                "request": {
                    "model": model,
...
def prepare() -> None:
    ...
    print("Prepared four prompt-safe blue-gate action payloads")
...
async def call_one(case: dict[str, Any], repeat: int, cfg: dict[str, Any]) -> None:
    ...
    try:
        ...
        if not quote or not any(quote in source for source in case["event_texts"]):
            raise ValueError("evidence_quote is not a nonempty literal event substring")
...
def grade() -> None:
    ...
    content = all(
        len(per_turn[turn]) == 4
        and all(row["closing_action"] == expected for row in per_turn[turn])
        for turn, expected in ((36, False), (37, True), (38, True), (39, False))
    )
