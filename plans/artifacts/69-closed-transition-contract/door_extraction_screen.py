...
PREREG = HERE / "DOOR-EXTRACTION-V2-PREREGISTRATION.md"
MANIFEST = HERE / "door-extraction-manifest.json"
RUNS = HERE / "door-extraction-runs"
...
SCHEMA: dict[str, Any] = {
    "type": "object",
    "properties": {"aperture": {"type": "string", "enum": list(LABELS)}},
    "required": ["aperture"],
...
def source_rows() -> dict[int, tuple[int, dict[str, Any]]]:
    ...
    if set(selected) != set(TURNS):
        raise RuntimeError(
            "exactly one accepted first-attempt Director response per T7, T8, T10 is required"
        )
    return selected
...
def prompt(turn: int, projection: dict[str, Any]) -> list[dict[str, str]]:
    current = "ajar" if turn in (7, 8) else "open"
    candidate = "closed" if turn == 10 else "open"
    catalogue = {
        "main_hall_gates": {
            "label": "main hall gates",
            "property": "aperture",
    ...
    system = (
        ...
        "Return exactly one of change, no_change, uncertain for the aperture property. "
...
def prepare() -> None:
    ...
    print("Prepared three prompt-safe public door payloads")
...
async def run() -> None:
    ...
    if (
        ...
        or manifest["session_id"] != "5c994c42"
    ):
...
def grade() -> None:
    ...
    technical = len(results) == 12 and len(valid) == 12
    content = all(
        len(per_turn[turn]) == 4 and all(row["aperture"] == expected for row in per_turn[turn])
        for turn, expected in ((7, "no_change"), (8, "change"), (10, "no_change"))
    )
